...
{
    [SerializeField] string _aimTargetName;
    [SerializeField] [Range(50, 90)] private float rotationThreshold;
    private Transform aimTarget;
    // OnStateEnter is called when a transition starts and the state machine starts to evaluate this state
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        aimTarget = animator.GetComponentsInChildren<Transform>().FirstOrDefault(transform => transform.gameObject.name == _aimTargetName);
    }
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        Vector3 aimForward = Vector3.ProjectOnPlane(aimTarget.forward, animator.transform.up).normalized;
